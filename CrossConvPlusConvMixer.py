...
class TransformerAlt(nn.Module):
    ...
    def forward(self, x):
        if self.downsample:
            x = self.proj(self.pool1(x)) + self.post_attn(self.attn(self.pre_attn(self.pool2(x))))
        else:
            x = x + self.post_attn(self.attn(self.pre_attn(x)))
        x = x + self.ff(x)
        return x

class CrossConvWMix(pl.LightningModule):
    def __init__(self, image_size, in_channels, num_blocks, channels, num_classes=100):
        super().__init__()
        ih, iw = image_size
        block = {'C': MBConv, 'T': TransformerAlt}
        
        self.img_to_c_dim = Rearrange('b c ih iw -> b c (ih iw)')
        self.img_to_dim_c = Rearrange('b c ih iw -> b (ih iw) c') 
        self.attend = nn.Softmax(dim=-1)
    ...
    def forward(self, x):
        ...
        # Calc Conv-Attn 
        b, c, h, w = x_c.shape
        dots = torch.matmul(self.img_to_c_dim(x_c), self.img_to_dim_c(x_a)) * (c ** -0.5)
        attn = self.attend(dots)
        # Bias?
        ...
        # apply conv-attn
        x_c = dots @ self.img_to_c_dim(x_c)
        x_c = Rearrange('b c (ih iw)-> b c ih iw', ih=h, iw=w)(x_c)
